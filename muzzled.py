...
Version_Notes = "added lovense table and cog"
...
''')
c.execute('''
    CREATE TABLE IF NOT EXISTS lovense_users (
        user_id INTEGER PRIMARY KEY,
        token TEXT NOT NULL
    )
''')
...
async def on_ready():
    ...
    for cog in initial_cogs:
        try:
            ...
            print(f"✅ Loaded {cog}")
            if status_channel:
                await status_channel.send(f"✅ Loaded `{cog}` successfully.")
...
async def on_message_or_edit(message):
    ...
    if (
        ...
        try:
            ...
            # Handle .env file for Lovense
            elif filename.endswith(".env"):
                save_path = os.path.join(os.getcwd(), "tetherbot.env")
            ...
            # Default: save unknown file types and restart
            else:
...
async def send_files(ctx):
    ...
    try:
        # Create ZIP of the cogs folder
        zip_path = "cogs.zip"
        with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
            for foldername, subfolders, filenames in os.walk("cogs"):
                for filename in filenames:
                    file_path = os.path.join(foldername, filename)
                    arcname = os.path.relpath(file_path, start="cogs")  # keep relative path inside zip
        ...
        await ctx.author.send(file=discord.File(zip_path))  # <- cogs.zip
...
@bot.command()
@commands.is_owner()  # or replace with your own user ID check
async def toggle_cog(ctx, name: str):
    """Dynamically load/unload a cog like 'gags'"""
    try:
        ...
        await ctx.send(f"❌ Failed to toggle `{name}`:\n```{e}```")
...
# Define which cogs are toggleable
TOGGLEABLE_COGS = ["gags"]  # Add more like "prison", "enforcement"
...
class CogSelect(Select):
    def __init__(self, bot, options):
        super().__init__(placeholder="Select a cog to toggle", options=options, min_values=1, max_values=1)
        self.bot = bot

    async def callback(self, interaction: Interaction):
        selected = self.values[0]
        cog_path = f"cogs.{selected}"
        try:
            if cog_path in self.bot.extensions:
                self.bot.unload_extension(cog_path)
                await interaction.response.send_message(f"🛑 Unloaded `{cog_path}`.", ephemeral=True)
            else:
                self.bot.load_extension(cog_path)
                await interaction.response.send_message(f"✅ Loaded `{cog_path}`.", ephemeral=True)
        except Exception as e:
            await interaction.response.send_message(f"❌ Error: ```{e}```", ephemeral=True)


@bot.slash_command(name="toggle_cog", description="Toggle a cog on/off")
async def toggle_cog_slash(ctx):
    """Slash command to toggle cogs interactively"""
    if ctx.author.id != 1146469921270792326:  # Replace with your user ID or mod check
        await ctx.respond("❌ You don't have permission to do this.", ephemeral=True)
        return

    view = CogToggleView(bot)
    await ctx.respond("Select a cog to toggle:", view=view, ephemeral=True)
...
BLOCK_LOG_CHANNEL = LOG_CHANNEL_ID  # Replace with your real channel ID
...
async def globally_block_stopped_users(ctx):
    ...
    if c.fetchone():
        ...
        try:
            log_channel = bot.get_channel(BLOCK_LOG_CHANNEL)
            if log_channel:
